...
The hiring panel contains Chakra, Interview, and All Products in one horizontally scrollable, snap-aligned row. Each card occupies most of the viewport width so one product remains legible while the next card peeks into view. The developer panel contains Community as one full-width card. Activating Developers selects Community and shows the Community panel immediately. Activating Hiring restores the last hiring selection and its demo form. On phone widths the Schedule A Demo button participates in normal form flow, preventing it from covering fields while scrolling.
